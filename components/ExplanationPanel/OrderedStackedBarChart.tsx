...
interface OrderedStackedBarChartProps {
  rankBy: string;
...
const OrderedStackedBarChart = ({
  rankBy,
  weights,
  className = "",
  hoveredCriterion: externalHoveredCriterion = null,
  onCriterionHover,
}: OrderedStackedBarChartProps) => {
  ...
  // Calculate available space above and below 0-level, accounting for item images at top
  const zeroLevelPosition = height * 0.6; // Position 0-level lower to leave more space for item images at top
  const spaceAboveZero = (zeroLevelPosition - 50) * 0.95; // Reserve 50px for item images at top, use 95% of remaining space
  const spaceBelowZero = (height - zeroLevelPosition) * 0.98; // Use 98% of space below zero level
        ...
        >
          <g transform={`translate(${margin.left}, ${margin.top})`}>
            ...
            {stackedData.map((item, index) => {
              const x = scales.xScale(item.startRank); // Use startRank for OrderedStackedBarChart
            ...
            {/* Vertical stacked bars positioned above top row items */}
            {stackedData.map((item) => {
              const x = scales.xScale(item.startRank); // Use startRank for top row positioning
...
export default OrderedStackedBarChart;
